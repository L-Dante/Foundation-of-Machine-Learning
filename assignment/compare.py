...
while epoch < max_iter and cent_move > epsilon:
    ...
    for i in range(k):
        ...
        #plt.scatter(cent_cur[i][0],cent_cur[i][1])
    cent_move = np.linalg.norm(cent_cur-cent_pre)
    
plt.scatter(cent_cur[:, 0], cent_cur[:, 1], c='k', label='my kmeans')
km = KMeans(n_clusters=3)
km.fit(X)
centroids = km.cluster_centers_
plt.scatter(centroids[:,0], centroids[:,1], c='g', label='sklearn kmeans')
